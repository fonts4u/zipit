...
async function getBrowser() {
  ...
  if (IS_VERCEL && chromium) {
    ...
    // Local dev OR Railway/Docker — use system Chromium / Chrome
    const findChrome = () => {
      if (process.platform === 'win32') return 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
      if (process.platform === 'darwin') return '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';

      // Linux: Dynamic search
      const paths = [
      ...
      // Try 'which' as a last resort for Railway/Nixpacks
      try {
        const { execSync } = require('child_process');
        const resolvePath = (cmd) => execSync(`which ${cmd}`, { encoding: 'utf8' }).trim();
        return resolvePath('google-chrome-stable') || resolvePath('chromium') || resolvePath('google-chrome');
      } catch (e) {
        return 'chromium'; // final fallback
      }
    };

    const executablePath = process.env.CHROMIUM_PATH || findChrome();
    const logger = require('./logger');
    logger.info(`Attempting to launch browser from: ${executablePath}`);
...
const PAGE_TIMEOUT_MS = 15_000; // reduced for serverless
...
async function crawlPage(url) {
  ...
  try {
    ...
    // Navigate — fast load for Vercel, networkidle2 for Railway/Local
    await page.goto(url, {
      waitUntil: IS_VERCEL ? 'load' : 'networkidle2',
      timeout: IS_VERCEL ? 8000 : PAGE_TIMEOUT_MS,
    });

    // Extra wait for any post-idle dynamic injections
    await new Promise((r) => setTimeout(r, 1_000));

    // Only skip scroll on Vercel; it takes too long for 10s limit.
    // Railway and Local dev have enough time.
    if (!IS_VERCEL) {
