...
async function getBrowser() {
  ...
  if (IS_VERCEL && chromium) {
    ...
    const findChrome = () => {
      ...
      // Linux: Check common paths for Railway/Docker/Serverless
      const paths = [
      ...
      return 'chromium'; // last-resort fallback to PATH
    };

    const executablePath = process.env.CHROMIUM_PATH || findChrome();
    const logger = require('./logger');
    logger.info(`Launching browser from path: ${executablePath}`);
