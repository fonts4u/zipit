...
async function crawlPage(url) {
  ...
  try {
    ...
    // Navigate — fast load for serverless
    await page.goto(url, {
      waitUntil: isServerless ? 'load' : 'networkidle2',
      timeout: isServerless ? 8000 : PAGE_TIMEOUT_MS,
    });

    // Extra wait for any post-idle dynamic injections
    await new Promise((r) => setTimeout(r, 1_000));

    // Only scroll on local dev; it takes too long for 10s Vercel limit
    if (!isServerless) {
